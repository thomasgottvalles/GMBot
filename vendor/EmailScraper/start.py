...
def main():
    ...
    print("Opening the file containing Google Maps data in all.json")
    with open("../../output/all.json",'r+') as json_file:
        json_data = json.load(json_file)
        for element in json_data:
            
            # Checks data in the file.
            print("Checking if website exists from " + element["title"])
            if element["website"] is not None:
                ...
                if not "emails_extracted" in element and not "contact_email" in element:
                    ...
                    print("Website found! Starting the bot and scraping emails from " + domain)
                    command = "scrapy crawl emailscraper -a domain=" + domain + " -o " + element["place_id"] + "." + domain + ".json"
                    subprocess.run(command, shell=True)

                    # Counts emails and selects the most relevant contact email.
                    print("Emails counting and contact email selection in " + element["place_id"] + "." + domain + ".json")
                    json_file_website = open(element["place_id"] + "." +domain + ".json")
                    ...
                    print("Emails extrated in " + element["place_id"] + "." + domain + ".json")
                    ...
                    print("Contact email selected: " + str(contact_email))
